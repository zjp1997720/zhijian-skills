...
description: 在 Codex App 中为复杂、可并行的知识工作或编程任务创建可指定模型与推理强度的后台任务，由主 Agent 负责规划、provider 数据边界、分工、集成和验收。用于多来源调研、多章节内容、复杂 Skill/PPT、跨模块开发、独立验证或 2 个以上互不依赖工作流；也用于用户明确要求模型路由、后台 Worker、Agents Team、Grok/Gemini Worker 或持久项目协作。简单问答、状态查询、单文件小改、强顺序任务和发布/付款/删除/账户操作不得自动触发。
---
...
4. 按 [任务生命周期](references/thread-lifecycle.md) 做分层预检：注册表与 live runtime 元数据负责静态可接受性；Provider 策略负责账号、条款和数据边界；可用时运行不占 App Thread 的语义 canary；每个新 `host/model/thinking/tool-signature` 的第一个真实业务 Worker 负责最终数据面探针。缺少任一层证据时不得把局部检查写成 `route_eligible=true`。
5. 派遣前显示一条简短通知：任务数、每个任务的精确模型、`thinking`、职责、预声明 fallback，以及为后续阶段和重试预留的累计额度。Gemini Antigravity 路径必须说明 manual-only 状态和账号风险。
6. 读取 [任务包模板](references/task-packet.md)，为每个 Worker 写独立可执行的完整提示词。提示词必须包含“禁止创建任何后台任务、线程或子 Agent”。
7. 用 `codex_app__list_projects` 定位项目；任何声明工作区输出路径的任务都使用匹配 project local，只有纯聊天交付才能 projectless。调用 `create_thread` 前先消耗一次 root `creation_attempt` 和 subtask attempt，并按 [审计 schema](references/audit-schema.json) 建立 `thread_id: null` 的 ledger 记录；再显式传入 RoutePlan 的 `model` 与 `thinking`。
8. `create_thread` 返回正式 `threadId` 后立即写回 ledger，再用 `codex_app__read_thread` 验证实体化。读到真实 thread、cwd 与 turn 状态后标记 `materialized=true`。首个 assistant-originated 输出项或模型发起的工具调用出现后标记 `data_ready=true`；用户消息、Thread 元数据和 MCP 启动错误不计入 `DATA_READY`。
...
11. 主 Agent 亲自核对事实、运行验证、处理冲突并整合最终交付。只对已实体化、completed/idle、输出文件已经验证、且结果已采纳的轻量任务逐个调用 `codex_app__set_thread_archived`；失败、争议或待审任务保留。
12. 所有 Worker 记录遵守 [审计 schema](references/audit-schema.json)，包含 `creation_attempt / subtask_attempt / thread_id / role / model / requested_model / platform_accepted_model / observed_runtime_model / thinking / route_plan / provider_policy / materialized / data_ready / status / output / adopted / fallback_reason / archived`。`model` 保持为 `requested_model` 的兼容别名；平台没有回显真实模型时，`observed_runtime_model` 记为 `unknown`。最终汇报模型分布、预检、重试、升级、采纳、归档和未解决风险。
...
- `create_thread` 超时后产生的未实体化 ID 不是可管理任务。禁止恢复、追问或归档该 ID，也禁止直接修改 Codex 数据库；记录故障并停止同批创建。
...
交付必须完整、自洽、经过主 Agent 验证，并包含可审计的模型路由摘要。上游 Skill 模式另外报告 reserved slots、阶段门、输出采纳和归档状态。触发、并发、provider、恢复和归档边界见 [验证案例](references/validation-cases.md)。
