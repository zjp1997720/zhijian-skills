...
ensure_state_root
[ -d "$INSTALL_ROOT" ] || fail "Dream Skin install is missing: $INSTALL_ROOT"
...
if codex_is_running; then
  verified_cdp_endpoint "$PORT" \
    || fail "Codex is running but the managed CDP endpoint cannot be verified; V2 restore stopped before file exchange."
  "$NODE" "$INJECTOR" --remove --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 8000 >/dev/null \
    || fail "The live V3 skin could not be removed; V2 restore stopped before file exchange."
fi
