...
3. 每种精确组合的第一个真实业务任务通过实体化和数据面门后，才释放同组合后续任务。
4. 后续任务按受控批次创建，并明确模型、推理强度、范围、文件所有权和验收标准。
5. 主 Agent 亲自核验事实与产物、处理冲突、按错误类别恢复并完成最终交付。
6. 已采纳且完成的任务逐个归档。
...
工作流已经覆盖 Luna、Sol、条件 Grok 4.5 路由，以及 Gemini 显式请求的 Provider 阻断；包含 registry/runtime 检查、语义 nonce、RoutePlan 校验、分类恢复和串行归档。发布包还会在隔离环境中执行一次真实的 `npx skills` 安装，确认配套文件被完整复制。
