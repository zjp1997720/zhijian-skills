...
- 同时运行最多 6 个后台任务；单个根任务最多发起 8 次后台创建尝试，失败、未实体化和 fallback 也计数。后台任务不得再创建任何后台任务或子 Agent。
...
- Luna 与 Sol 提供稳定基线；Grok 4.5 在 runtime/provider 预检通过后可以承担复杂编码、终端执行和异构审查。
- Gemini 3.6 Flash 保留显式路由模板，但当前 Antigravity 第三方登录路径受官方条款阻断；正式 API/Vertex 路径需要新的 registry entry。
- 每波最多新增 3 个任务，同时运行最多 6 个，单个根任务最多发起 8 次创建尝试。
...
- fallback 在派遣前固定；每个子任务最多两个 Worker Thread，完整输出最多在原 Thread 追问一次。
- 可以作为 Deep Research 等上游 Skill 的 Thread Orchestrator，保留上游流程、阶段门、产物和质量标准。
- 发布、付款、删除、账户操作和生产变更始终由主 Agent 执行。

## 工作方式

1. 主 Agent 判断并行收益是否高于协调成本，并固定任务画像、Provider allowlist 和有序候选链。
2. 静态检查 registry、live runtime、thinking 和 Provider 门；可用时运行不占 App Thread 的语义 nonce 探针。
3. 每次创建写入唯一 task id；直接返回正式 id 时读取实体化，只返回 pending id 时通过 `list_threads` 恢复并稳定确认 Thread 身份。
4. 每种精确组合的第一个真实业务任务通过实体化和数据面门后，才释放同组合后续任务。
5. 主 Agent 以最新官方读取恢复状态、亲自核验产物并按错误类别处理；`UNKNOWN` 不猜测性重建。
6. 已采纳、完成且通过归档门的任务逐个归档。
...
- Codex App 能够提供项目定位、后台任务创建、任务查询、任务读取、追问和归档工具。
...
工作流已经覆盖 Luna、Sol、条件 Grok 4.5 路由，以及 Gemini 显式请求的 Provider 阻断；包含 registry/runtime 检查、语义 nonce、RoutePlan 校验、pending worktree 恢复、权威状态重建、ledger 校验和串行归档。发布包还会在隔离环境中执行一次真实的 `npx skills` 安装，确认配套文件被完整复制。
